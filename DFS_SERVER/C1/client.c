...
int get_file(char *host, char *name, int serverno) //just to ensure all servers are not operating on same domain, we use serverno.
{
	...
	while (1) 
	{
		...
		for(i=0;i<5;i++)
		{
			if(cur_ts[i] <= result->readfile_res_u.part.ts[i])
...
void sendfile(char *host, char *name, int serverno)
{
	...
	if(updateResult != 0)
	{
		printf("Server %d is down with error number %d\n",serverno,updateResult);
		printf("Try server %d.\n",(serverno+1)%3);
	}
}
...
int list_files(const char* host,int serverno)
{
	...
	if(result->listfile_res_u.fileListing.len == 0)
		printf("No files found on this server.\n");
	else
...
int read_command()
{
	...
	else
	{
		...
		{
			retval = get_file(getHost(serverno),filepath,serverno);
		} 
		else if(strcmp(command, "put") == 0)
		{
			cur_ts[Id-1]++;
			setClock();
			sendfile(getHost(serverno),filepath,serverno);
			retval = 0;
		}
		else
		{
			printf("The available commands are : \nlist\nexit\nget <filename> <servernum>\nput <filename> <servernum>\n");
			retval = -1;
		}
		//end critical section
		int release = -1;
